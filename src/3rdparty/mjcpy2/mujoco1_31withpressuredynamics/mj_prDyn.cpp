...
double pressureLocal(const mjModel* m, const mjData* d, int id)
{
	...
	mjtNum act = pr_model_prm[0] + d->act[id]; // act = pressure - c0;
	...
	{
		printf("Pressure (%0.2f) under bounds (%10.2f). Adjusting to %0.2f\n", act, act-pr_model_prm[0], pr_model_prm[0]);
		act = pr_model_prm[0];
	}
	else if (act>pr_model_prm[1])
	{
		printf("Pressure (%0.2f) over bounds (%10.2f). Adjusting to %0.2f\n", act, act-pr_model_prm[1], pr_model_prm[1]);
		act = pr_model_prm[1];
	}
