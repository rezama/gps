...
//uses the gompertz function. params(5x1)
mjtNum area_gompertz(mjtNum u, const mjtNum* params, mjtNum* dareadu)
{
	mjtNum ar = params[0] + params[1] * exp(-1.0*params[2] * exp(-1.0*params[3] * (u + params[4])));

	if (dareadu)
		dareadu[0] = params[1] * params[2] * params[3] * exp(-1.0*params[2] * exp(-1.0*params[3] * (u + params[4])))*exp(-1.0*params[3] * (u + params[4]));
	if (ar<0)
	{
		printf("Negative area. Returning 0\n");
		return 0.0;
...
// c[15] = {p_a, p_c, leakArea, {gompertz}_a[5], {gompertz}_c[5], {volume}[2]}
	...
	mjtNum dt, mjtNum* dpdz)
{
	...
	const mjtNum* params_a = NULL;		// atmospheric port area params {Yoffset, scale, Xoffset, r}_a
	const mjtNum* params_c = NULL;		// compressor port area params {Yoffset, scale, Xoffset, r}_c
	const mjtNum *params_v;             // length to volume params
	...
	if(u!=u)	 // bad voltage, default to constant dynamics
		return p;
	else
	{
		// mark derivatives
        ...
        // area 
        mjtNum vol = x*boreArea;
	    mjtNum vDot = boreArea;
	    mjtNum leakArea = 0.0;
		areaFcn areaFcn_inUse = gompertz;
 		switch(areaFcn_inUse)
		{
			...
			case gompertz:
				...
				params_c = c + 8;
		        params_v = c + 13;
				ar_c = area_gompertz(u, params_c, darea_cdu);
				ar_a = area_gompertz(u, params_a, darea_adu);
                leakArea = c[2];
		        vol = (params_v[0] * (0.200 - x)) + params_v[1];
		        vDot = -params_v[0] * v;
				break;
			default: 
        ...
        dm = ar_c*f_c - (ar_a + leakArea)*f_a;

		// dp = n*(Rs*T*dm - p*dv)/v
		dp = n*(Rs*T*dm - p*vDot)/(vol);
